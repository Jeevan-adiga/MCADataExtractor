...
import auto.framework.drivers.DriverReportListener;
...
//		}
	...
	public static class Listener extends  TestNG.ExitCodeListener implements ITestListener {
	    ...
	    public void onTestStart(ITestResult result) {
			...
			if(executeScript){				
				...
				try {
					...
					//setStatus(result, ITestResult.STARTED);	
					setStatus(result, ITestResult.STARTED);	
				} catch (Exception e){
				}
			}
		...
		public void setStatus(ITestResult tr, int status){
			
			if(tr.getTestContext().getName()!=null){
				try {
					WebDriver driver = WebManager.getRemoteDriver();
					if(driver==null){
						System.err.println("Driver not found "+status);
						return;
					}
					if(driver instanceof DriverReportListener){
						DriverReportListener driverListener = (DriverReportListener) driver;
						driverListener.setTestStatus(tr, status);
					} else {
						//System.err.println("Driver not listener "+status);
					}
				} catch(Exception e){ e.printStackTrace(); }
			} else {
				System.err.println("Test name not found "+status);
			}
		}
