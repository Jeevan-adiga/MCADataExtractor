...
public class Element implements ISearchContext {
	...
	/*public static Element toElement(WebElement element){				
	...
	*/
	...
	protected Boolean isInteractive(Boolean log){
		try {
			this.waitForDisplay(true, 10,log);
			return this.isDisplayed() && this.isEnabled();
		} catch(Error error){
			return false;
		}
...
//	}
	...
	public Boolean waitForDisplay(final Boolean expected,long timeOutInSeconds,Boolean log){
		Boolean success = waitForProperty(Element.DISPLAYED, expected, timeOutInSeconds);
		Boolean detailLog = Boolean.valueOf(TestManager.Preferences.getPreference("detailedLogging"));
	...
	protected void fireIEEvent(String event){
		WebDriver driver = WebManager.getDriver();
		Capabilities caps = WebManager.getCapabilities();
...
//		}
	...
	protected static String getEventScript(String eventName){
	...
	protected static String getMouseEventScript(String eventName){
		String script = "var element = arguments[0];"
			+ "var event; // The custom event that will be created\n"
			+ "if (document.createEvent) {\n"
	...
	public Boolean clickAfterScroll(){
		scrollIntoView();		
		return click();
	}
	...
	public Boolean click(Boolean Report){
		Boolean detailLog = Boolean.valueOf(TestManager.Preferences.getPreference("detailedLogging"));
		Boolean prereq = isInteractive(Report);
		Boolean success = false;
		if(prereq) {
			try{
				doClick();
...
//sendKeys
	...
	public void scrollIntoView(){
		try {
			((JavascriptExecutor) WebManager.getRemoteDriver()).executeScript("arguments[0].scrollIntoView(true);", this.toWebElement());
			ReportLog.passed("[" + name + "] Scroll into browser's window view" );
		} catch(NoSuchElementException |Error e){
			ReportLog.failed("[" + name + "] Scroll into browser's window view" );
		}
	}
	
	public void scrollIntoView(Boolean log){
		try {
			((JavascriptExecutor) WebManager.getRemoteDriver()).executeScript("arguments[0].scrollIntoView(true);", this.toWebElement());
			if(log){
				ReportLog.passed("[" + name + "] Scroll into browser's window view" );	
	...
	public Boolean sendKeysPwd(String string){
		Boolean prereq = isInteractive(); 
		Boolean success = false;
		if(prereq) { 
		...
		ReportLog.logEvent(success, "[" + name + "] Type password \"********\"");
		return success;
	}
	...
	public void sendKeysByJS(String string) {
		try {
			JavascriptExecutor exec = (JavascriptExecutor) WebManager.getDriver();
			exec.executeScript("arguments[0].setAttribute('value','"+string+"')", toWebElement());						
			ReportLog.passed("[" + name + "] Type \"" + string + "\"");
			} 
		catch(TimeoutException e) {
			ReportLog.failed("[" + name + "] Type \"" + string + "\"");
			}		
	}
...
//tagName
	...
	public void scrollToElement(int viewportX, int viewportY) {
		...
		if(frame!=null){
			...
			Point scroll2 = element.getLocation();
...
//selected
	public boolean isSelected() {
	  ...
	  public  boolean isClickable(int timeOut) {
	        ...
	        WebElement element = toWebElement();
	        try {
	            final WebDriverWait wait = new WebDriverWait(WebManager.getRemoteDriver(),timeOut);
